# Feature 0007 - Band Members and Roster

**Status:** In Refinement

## Navigation
- [Diagrams](diagrams/README.md)
- [Planning overview](planning/overview.md)
- [Design (data model + cast)](planning/design.md)
- [Decomposition](refinement/decomposition.md)
- [Checklist](refinement/checklist.md)
- [Open questions](refinement/questions.md)
- [Latest iteration](refinement/iteration-04.md)
- [Action log](refinement/log.md)
- [Handoff — character icons](handoff/character-icons-handoff.md)

## Relationships
- **0006 (Art Direction)** — dona do *card visual* do membro e do estilo de
  ilustração (vetorial, pose fixa).
- **0005 (Start Screen and Onboarding)** — dona do *fluxo de criação da banda*.
- **0002 (Progressão da Banda)** — trata de fases de carreira, não do roster.
- **Esta feature (0007)** — dona do *modelo de membro* e da *formação inicial*.
